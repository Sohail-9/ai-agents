...
npm run worker                 # all workers (set WORKER_KIND=agent|setup|import|github-sync|reaper|prewarm|billing|support|all)
npm run build                  # tsc + copy Prisma artifacts to dist/ + copy skill assets
...
**Targeted test scripts (backend):** standalone tsx scripts, not a test runner.
```bash
...
npm run bench:queue            # queue throughput benchmark
```
...
| Path | Purpose |
|------|---------|
| `backend/src/brain/` | LLM orchestration, prompt construction, model routing, multi-agent coordination |
| `backend/src/workers/` | BullMQ workers split by kind: `agent`, `setup`, `import`, `github-sync`, `reaper`, `prewarm`, `billing`, `support`, `cleanup` |
| `backend/src/skills/` | Tool implementations — file ops, shell, code intelligence, web, database provisioning, architect, plan, frontend-design, env, todo |
...
Workers run as separate PM2 processes (see `backend/ecosystem.config.cjs`). The `WORKER_KIND` env var controls which queues a worker instance subscribes to (`all` = dev default, registers every worker). Prod layout uses dedicated processes for agent/setup/import/coregit/github-sync/reaper/prewarm/support/billing, so a long-running agent job cannot starve setup/import jobs.

### Multi-Model LLM

Supports Anthropic Claude (primary), OpenAI, Groq, Gemini, and Qwen (DashScope). Model routing is inside `backend/src/brain/` (`modelSelector.ts`, `providers/`). Key env vars: `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GROQ_API_KEY`, `GOOGLE_GENAI_API_KEY`, `QWEN_API_KEY`.
...
Two DB connection strings in env: `DATABASE_URL` (standard, pooled) and `DATABASE_URL_DIRECT` (direct non-pooled, required for migrations).
