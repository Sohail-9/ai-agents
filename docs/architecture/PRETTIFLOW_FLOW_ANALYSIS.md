# Prettiflow Query → Prettiflow.md Flow Analysis

## Context

This document maps the complete code path from a user query to the `Prettiflow.md` file inside the E2B sandbox, and how that file is read before every agent loop iteration.
...
| `backend/src/workers/setupWorker.ts` | BullMQ worker — builds and writes Prettiflow.md |
...
| `backend/src/brain/agentRunner.ts` | Main agent loop — instructs agent to read Prettiflow.md |
| `backend/src/brain/systemPrompt.ts` | System prompt construction — Prettiflow.md read instructions |
...
## Phase 2: TOON Plan → Prettiflow.md in Sandbox

### 4. setupWorker picks up the job
`setupWorker.ts:116-133` — `processSetupJob()`

**Fast path** (`setupWorker.ts:180-181`): `cachedAiResponse.contextContent` present → used directly as `prettiflowMd`, no new LLM call.

**Slow path** (`setupWorker.ts:183-208`): no cache → calls `ai.processPrompt()` again.

### 5. Write Prettiflow.md to sandbox
`setupWorker.ts:228-235` calls `SandboxManager.openAndInit({ prettiflowMd, ... })`

Inside `sandboxManager.ts`:
- Line 62-119: Creates or connects to E2B sandbox via `Sandbox.create()` / `Sandbox.connect()`
- **Line 128**: `sandbox.files.write("/workspace/Prettiflow.md", input.prettiflowMd)`
- Line 158: `await Promise.all(fileWrites)` — write executed here

### 6. Persist to DB + parse todos
`setupWorker.ts:260-265`:
- `workspaceService.updatePrettiflow(workspaceId, prettiflowMd)` → saves to DB
- `parseTodosFromContext(prettiflowMd)` (`planningUtils.ts:43-186`) → parses `TODOS` section
- `todoService.createTodosWithDeps(workspaceId, todos, 1)` → saves todos to DB
- Enqueues **`agent-run`** BullMQ job

---

## Phase 3: Prettiflow.md Read Before Every Loop
...
System prompt locations that instruct reading Prettiflow.md:
- `systemPrompt.ts:380-381` (prebuilt Next.js path): `"Start by reading /workspace/Prettiflow.md"`
- `systemPrompt.ts:441-442` (fallback path): `"First, check if Prettiflow.md exists..."`
- `systemPrompt.ts:555-556` (plan mode): `"cat /workspace/Prettiflow.md if it exists"`
- `systemPrompt.ts:290` (SHARED_RULES, all paths): `"Prettiflow.md contains everything you need"`

### 9. Wave task message — per-loop injection
`agentRunner.ts:733-734` inside `buildInitialWaveMessage()`, injected at line 1402-1406:
```
"- Knowledge Base: Read /workspace/Prettiflow.md ONCE for the full spec. Then go straight to writing code"
```
This fires **at the start of every wave** (outer loop iteration).

### 10. Agent reads Prettiflow.md via tool call
The agent issues a `read_file` tool call targeting `/workspace/Prettiflow.md` in the sandbox.

**Prettiflow.md content is NEVER injected into the prompt.** The agent reads it from the sandbox filesystem each time it starts a new wave.
...
Outer wave loop: while (totalIterations < 120)       agentRunner.ts:1211
  ...
  ↓ buildInitialWaveMessage() → injects Prettiflow.md read instruction
  ↓
...
- `buildUpdateContext(workspace)` (`planningUtils.ts:189-208`) reads `prettiflowMd` **from DB** (not sandbox)
- This is passed to `ai.planUpdate()` for re-planning
- Updated todos created in DB
- New `agent-run` job enqueued
- Agent reads `/workspace/Prettiflow.md` from sandbox as normal

---

## TOON Format (Prettiflow.md)
...
WSManager USER_REQUEST (WSManager.ts:728)
    ...
    │       └── sandbox.files.write("/workspace/Prettiflow.md") (sandboxManager.ts:128)
    │   └── workspaceService.updatePrettiflow() → saves to DB
    ...
    │   └── buildUpdateContext() reads prettiflowMd from DB
...
agentWorker.ts → runOrchestrator() → runAgent() (agentRunner.ts)
    ...
    │   └── includes "read /workspace/Prettiflow.md" instructions
    │
    └─ Wave loop (max 120 iterations)
        └─ buildInitialWaveMessage() → "Read /workspace/Prettiflow.md ONCE"
        └─ Inner loop (max 20 iterations per wave)
            └─ Agent issues read_file("/workspace/Prettiflow.md") tool call
